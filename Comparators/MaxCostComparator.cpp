...
// Constructor for MaxCostComparator class.
MaxCostComparator::MaxCostComparator() {
    // JSON file path to be read.
    const std::string& jsonFilePath = "Cost.json";
    // Read the JSON file.
    readJsonFile(jsonFilePath);
}

// Destructor for MaxCostComparator class.
MaxCostComparator::~MaxCostComparator() {}

// Custom comparator for comparing vectors of Point3D based on their maximum z-values.
bool MaxCostComparator::operator()(const std::vector<Point3D>& v1, const std::vector<Point3D>& v2) const {
    ...
    // Compare the values associated with z1 and z2 and return true if value1 is greater than value2.
    return value1 > value2;
}

// Read the JSON file and populate the jsonValues map.
void MaxCostComparator::readJsonFile(const std::string& jsonFilePath) {
